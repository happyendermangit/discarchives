    ...
    <>
      ...
      <div className="mt-10 mb-10">
        ...
        <div className="quests">
          {data.map((quest) => {
            const assetBaseUrl = `https://cdn.discordapp.com/assets/quests/${quest.config.quest_id}`;
            return (
              <div className="card w-96 bg-base-100 shadow-xl">
                <figure>
                  <img
                    src={`${assetBaseUrl}/${quest.config.assets.hero}`}
                    alt="Shoes"
                  />
                </figure>
                <div className="card-body">
                  <h2 className="card-title">
                    ...
                    {quest.config.messages.quest_name} Quest —{" "}
                    {quest.config.application_name}
                  </h2>
                  <p>
                    Stream {quest.config.qpplication_name} for{" "}
                    {quest.config.stream_duration_requirement_minutes} minutes
                    to get <b>{quest.config.messages.reward_name}</b>
                  </p>
                  <div className="card-actions justify-end">
                    <button
                      className="btn btn-primary"
                      onClick={() =>
                        window.open(quest.config.get_game_link, "_blank")
                      }
                    >
                      Get game
                    </button>
